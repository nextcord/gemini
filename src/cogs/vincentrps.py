...
class HelpButton(ui.Button["HelpView"]):
    ...
    async def callback(self, interaction: Interaction):
        if self.custom_id == f"{CUSTOM_ID_PREFIX}slashcmds":
            GIST_URL = (
                "https://gist.github.com/TAG-Epic/68e05d98a89982bac827ad2c3a60c50a"
            )
...
class HelpView(ui.View):
    def __init__(self):
        ...
        self.add_item(
            HelpButton("Nextcord", style=ButtonStyle.red, custom_id="nextcord")
        )
        self.add_item(HelpButton("Python", style=ButtonStyle.green, custom_id="python"))
        self.add_item(
            HelpButton(
                "Slash Commands", style=ButtonStyle.blurple, custom_id="slashcmds"
            )
        )
...
class ThreadCloseView(ui.View):
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        ...
        return (
            interaction.user.id == self._thread_author.id
            or interaction.user.get_role(HELPER_ROLE_ID)
        )
